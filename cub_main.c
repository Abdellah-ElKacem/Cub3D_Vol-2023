...
/*   Updated: 2023/10/16 21:54:11 by ael-kace         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (line[*i] == 'F')
		(*i += 1, put_colors_f(map, line, i));
	if (line[*i] == 'C')
		(*i += 1, put_colors_c(map, line, i));
}
...
int main(int ac, char **av)
{
	if (ac == 2)
	{
		...
		if (!map_intial->no || !map_intial->so \
			|| !map_intial->we || !map_intial->ea)
			(write(2, "Error : No Path Here :/\n", 25), exit(1));
		// if (map_intial->f.r == -1 || map_intial->c.r == -1)
...
// 		(*i)++;
// 	if (!ft_isdigit(line[*i]))
// 		return (write(2, "Error6 : of color path:/", 24), exit(1));
// 	while (line[*i])
// 	{
// 		start = *i;
// 		while (line[*i] != ',' && line[*i] != '\n')
// 		{
// 			if (!ft_isdigit(line[*i]))
// 				return (write(2, "Error 8: of color path:/", 24), exit(1));
// 			len++;
// 			(*i)++;
// 		}
// 		str = ft_strdup_plus(line, len, start, i);
...
// 	}
// 	if (v != 2)
// 		return (write(2, "Error9 : of color path:/", 24), exit(1));
// }